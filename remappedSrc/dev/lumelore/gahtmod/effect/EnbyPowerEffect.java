...
public class EnbyPowerEffect extends StatusEffect {
    ...
    private boolean canClimb(PlayerEntity entity) {
        ...
        Optional<BlockPos> closetBlock = BlockPos.findClosest(entity.getBlockPos().up(), 1, 0, b->entity.method_48926().getBlockState(b).isSolid());
        Optional<BlockPos> closetBlockUnder = BlockPos.findClosest(entity.getBlockPos(), 1, 0, b->entity.method_48926().getBlockState(b).isSolid());
        ...
        else if (closetBlockUnder.isPresent() && entity.method_48926().getBlockState(closetBlockUnder.get().down()).isSolid() && !entity.isOnGround()) {
            return true;
        }
        return false;
    }
